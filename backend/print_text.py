...
# 1. Полностью отключаем сборщик мусора на время тяжелого импорта
gc.disable()
...
# Принудительно очищаем мусор после импорта и включаем GC обратно
gc.collect()
gc.enable()
...
def prepare_label_data(text, config):
    ...
    if show_header and header_text:
        header_text = (
            f"{header_text}-" if force_left_header else f"-{header_text}"
        )
...
def draw_header(hdc, text, sizes, force_left):
    ...
    # Сохраняем старый дефолтный шрифт DC, чтобы избежать утечек памяти
    old_font = hdc.SelectObject(font_top)
    ...
    x = (
        sizes["margin_x"]
        if force_left
        else sizes["width_px"] - text_w - sizes["margin_x"]
    )
    y = sizes["margin_y"]

    hdc.TextOut(x, y, text)

    # Возвращаем старый шрифт и удаляем созданный gdi-объект
    hdc.SelectObject(old_font)
    font_top.DeleteObject()
...
def draw_main_text(hdc, text, sizes, y_start, is_underlined, is_bold):
    ...
    # Защита от бесконечного цикла, если текст пустой
    ...
    # Сохраняем исходный шрифт контекста устройства
    old_font = hdc.SelectObject(hdc.GetCurrentFont())

    # Цикл подбора максимального размера шрифта
    while True:
        # Защита от бесконечного роста (не прыгаем выше высоты всей этикетки)
        if optimal_height > sizes["height_px"]:
            break
        ...
        # Удаляем временный шрифт сразу, чтобы освободить дескриптор Windows
        test_font.DeleteObject()
    ...
    # Финальная отрисовка подобранным размером
    ...
    # Чистим за собой ресурсы Windows GDI
    hdc.SelectObject(old_font)
    final_font.DeleteObject()
...
def print_text(text):
    ...
    try:
        ...
        hdc.EndDoc()
        print(
            f"Отправлено. Шрифт: {opt_h}px. Жирный: {data['is_bold']}. Текст хедера: '{data['header_text']}'"
        )
