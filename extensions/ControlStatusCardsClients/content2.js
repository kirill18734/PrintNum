...
const work_url = [
  ...
  "http://127.0.0.1:5500/extensions/ControlStatusCardsClients/index.html",
];
...
async function run(list_btn) {
  list_btn.forEach(async (btn) => {
    try {
      await btn_click(btn);
    } catch (err) {
      console.error(`❌ Ошибка при выполнении действия: ${btn}`, err); // Лог ошибки при выполнении действия
      return false;
    }
  });
}
...
function elementWithTextExists(text) {
  const res = text.slice(5).trim().toLowerCase();
  const exists = [...document.querySelectorAll("button, [role='button']")].some(
    (el) => el.textContent.toLowerCase().includes(res)
  );
  return exists;
}
...
document.addEventListener("keydown", async (num) => {
  ...
  let is_run = false;
  // проверка на соответствие нужного url
  ...
  if (is_run) {
    if (num.key.length === 1) {
      ...
      for (key in list_commands) {
        if (buffer == list_commands[key].code) {
          handleCombination(list_commands[key].search_btn);
          break;
        }
      }
      buffer = "";
